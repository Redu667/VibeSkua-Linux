...
public sealed partial class AccountManagerViewModel : BotControlViewModelBase
{
    ...
    private void _LaunchAcc(string username, string password, string displayName = null, bool? withScript = null)
    {
        try
        {
            ...
            // Prefer the platform client launcher (Linux: relaunch this AppImage
            // in --client mode, tagged with the account name so army members are
            // distinguishable). Falls back to launching Skua.exe on Windows.
            IClientLauncher? launcher = Ioc.Default.GetService<IClientLauncher>();
            Process? process;
            if (launcher is not null)
            {
                process = launcher.LaunchClient(accountName, args);
            }
            else
            {
