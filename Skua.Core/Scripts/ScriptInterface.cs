...
public class ScriptInterface : IScriptInterface, IScriptInterfaceManager, IDisposable
{
    ...
    private Thread ScriptInterfaceThread;
    private readonly object _interfaceThreadLock = new();
        ...
        IScriptAccounts accounts)
    {
        ...
        ScriptInterfaceThread = CreateInterfaceThread();

        IScriptInterface.Instance = this;
    }

    private Thread CreateInterfaceThread() => new(() =>
    {
    ...
    {
        // Called on every game 'loaded' event — including RELOADS. A finished
        // .NET Thread object can never be restarted (Start() throws
        // ThreadStateException), so after the first reload every Initialize()
        // threw and killed the rest of the 'loaded' event pipeline. Recreate the
        // thread when the previous one has terminated.
        lock (_interfaceThreadLock)
        {
            if (ScriptInterfaceThread.IsAlive)
                return;
            if ((ScriptInterfaceThread.ThreadState & System.Threading.ThreadState.Unstarted) == 0)
                ScriptInterfaceThread = CreateInterfaceThread();
