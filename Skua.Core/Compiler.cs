...
public class Compiler : CSharpScriptExecution
{
    ...
    /// <summary>
    /// Salt for the compiled-script disk cache directory. Bumped when the Linux
    /// source transforms change so any already-poisoned cache dir is abandoned.
    /// This alone is fragile (a single build that writes a non-neutralized DLL
    /// re-poisons the new dir forever), so the real guarantee is that callers now
    /// hash the *neutralized* source — see <see cref="NeutralizeWindowsPlatformGates"/>
    ...
    /// <summary>
    /// Delete compiled-script caches from previous cache versions on startup.
    /// Users upgrade AppImages in place, so old <c>Cached-Scripts-*</c> folders
    /// linger and have repeatedly caused stale DLLs to be reused after a fix. The
    /// active version's folder is kept; every sibling is removed.
    /// </summary>
    private static void PurgeStaleCacheDirectories()
    {
        try
        {
            string scriptsDir = ClientFileSources.SkuaScriptsDIR;
            if (!Directory.Exists(scriptsDir))
                return;

            string keep = $"Cached-Scripts-{CacheVersion}";
            foreach (string dir in Directory.EnumerateDirectories(scriptsDir, "Cached-Scripts-*"))
            {
    ...
    public new Type? CompileClassToType(string code, int? cacheHash = null, ScriptLoadContext? loadContext = null, string? scriptName = null)
    {
        ...
        if (loadContext != null || !CachedAssemblies.ContainsKey(hash))
        {
            ...
            if (cachedAssemblyPath == null)
            {
                ...
                code = NeutralizeWindowsPlatformGates(code);
    ...
    {
        ClearErrors();
        string sourceWithNamespaces = NeutralizeWindowsPlatformGates(PrependNamespaces(source));
    ...
    /// <summary>
    /// On non-Windows, neutralize Windows-platform gates in script source. Many
    /// AQW scripts (CoreBots' <c>SkuaVersionCheckerAsync</c>) do
    /// <c>if (!OperatingSystem.IsWindowsVersionAtLeast(10)) { ShowMessageBox(...); stopBot }</c>,
    /// which would halt every script on Linux. The Linux port deliberately emulates
    /// the Windows environment those scripts assume (WinForms shim, bundled runtime,
    /// …), so we treat these platform probes as satisfied instead of letting them
    /// stop the bot. No-op on Windows, where the real checks should run.
    ///
    /// Public + static so ScriptManager can fold this transform into the disk-cache
    /// key (hash the neutralized source), guaranteeing a neutralized DLL never
    /// collides with a stale non-neutralized one. Idempotent — running it twice on
    /// already-neutralized source is a harmless no-op.
    /// </summary>
    public static string NeutralizeWindowsPlatformGates(string source)
    {
        if (OperatingSystem.IsWindows() || string.IsNullOrEmpty(source))
            return source;

        // OperatingSystem.IsWindowsVersionAtLeast(10[, ...]) -> true
        source = System.Text.RegularExpressions.Regex.Replace(
            source, @"OperatingSystem\s*\.\s*IsWindowsVersionAtLeast\s*\([^)]*\)", "true");
        // OperatingSystem.IsWindows() -> true
        source = System.Text.RegularExpressions.Regex.Replace(
            source, @"OperatingSystem\s*\.\s*IsWindows\s*\(\s*\)", "true");
        return source;
    }
